...
def play_board_until_win(
    remaining_numbers: Sequence[int], board: BingoBoard
) -> Tuple[int, int]:
...
for i, number in enumerate(numbers):
    if len(boards) == 1:
        print("Final board identified:")
        n, val = play_board_until_win(numbers[i:], boards[0])
        print(n * val)
        break
    ...
    boards = [board for board in boards if not board.check_for_winner()[0]]
